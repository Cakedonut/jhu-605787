...
function buildAndShowAboutHTML (aboutHtml) {
  switchAboutToActive();
  const randomRating = getRandom1to5();

  for (let i = 0; i < randomRating; i++) {
    aboutHtml = insertProperty(aboutHtml, `class${i+1}`, 'fa fa-star');
  }

  for (let i = randomRating; i < 6; i++) {
    aboutHtml = insertProperty(aboutHtml, `class${i+1}`, 'fa fa-star-o');
  }
